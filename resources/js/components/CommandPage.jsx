...
const inputStyle = { width: '100%', boxSizing: 'border-box', padding: '9px 12px', border: '1px solid #d1d5db', borderRadius: 8, fontSize: 13, outline: 'none', color: '#374151' };
const labelStyle = { display: 'block', fontSize: 13, fontWeight: 700, color: '#111827', marginBottom: 8 };
...
function DeviceMultiSelect({ devices, selected, onChange }) {
    ...
    return (
        <div ref={ref} style={{ position: 'relative' }}>
            ...
            <div onClick={() => setOpen(o => !o)}
                style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '9px 12px', border: `1px solid ${open ? '#6366f1' : '#d1d5db'}`, borderRadius: 8, background: '#fff', cursor: 'pointer', fontSize: 13, color: selected.length ? '#111827' : '#9ca3af' }}>
                ...
                <div style={{ position: 'absolute', zIndex: 20, top: '100%', left: 0, right: 0, marginTop: 4, background: '#fff', border: '1px solid #e5e7eb', borderRadius: 8, boxShadow: '0 6px 20px rgba(0,0,0,0.15)', maxHeight: 320, display: 'flex', flexDirection: 'column' }}>
                    ...
                    {selected.length > 0 && (
                        <div style={{ padding: '0 12px 6px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            <span style={{ fontSize: 11.5, color: '#6b7280' }}>{selected.length} selected</span>
                    ...
                    <div style={{ overflowY: 'auto', flex: 1 }}>
                        {filtered.length === 0 ? (
                            ...
                            <label key={d.imei} onClick={e => e.stopPropagation()}
                                style={{ display: 'flex', alignItems: 'center', gap: 9, padding: '8px 12px', fontSize: 13, cursor: 'pointer', color: '#374151' }}>
...
function ResponseModal({ record, onClose }) {
    return (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}
            onClick={onClose}>
            <div style={{ background: '#fff', borderRadius: 12, width: 560, maxHeight: '80vh', display: 'flex', flexDirection: 'column', boxShadow: '0 24px 64px rgba(0,0,0,0.3)' }}
                onClick={e => e.stopPropagation()}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 20px', borderBottom: '1px solid #f1f5f9' }}>
                    <h2 style={{ margin: 0, fontSize: 15, fontWeight: 700, color: '#0f172a' }}>Response — {record.imei}</h2>
                    <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#9ca3af', fontSize: 16 }}>✕</button>
                </div>
                <pre style={{ margin: 0, padding: 20, overflow: 'auto', fontSize: 12, fontFamily: 'monospace', color: '#374151', background: '#f8fafc' }}>
    ...
    return (
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', background: '#fff' }}>
            <div style={{ padding: '14px 20px 12px', borderBottom: '1px solid #e5e7eb', flexShrink: 0 }}>
                <h2 style={{ margin: 0, fontSize: 16, fontWeight: 700, color: '#111827' }}>Command</h2>
                <p style={{ margin: '4px 0 0', fontSize: 12.5, color: '#6b7280' }}>Send a command to one or many devices at once via TurboHive's batch command dispatch.</p>
            </div>
            ...
            <div style={{ flex: 1, overflow: 'auto', padding: '18px 20px' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                    <label style={{ ...labelStyle, marginBottom: 0 }}>Command Content</label>
                    <button onClick={() => historyRef.current?.scrollIntoView({ behavior: 'smooth' })}
                        style={{ padding: '6px 14px', background: '#fff', color: '#374151', border: '1px solid #d1d5db', borderRadius: 8, fontSize: 12.5, fontWeight: 600, cursor: 'pointer' }}>
                        🕐 History
                    </button>
                </div>
                ...
                {sendError && <div style={{ marginTop: 12, padding: '8px 12px', background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 6, fontSize: 12, color: '#991b1b' }}>{sendError}</div>}
                {sendResult && (
                    <div style={{ marginTop: 12, padding: '10px 12px', background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: 6, fontSize: 12.5, color: '#166534' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 16 }}>
                    <button onClick={handleClear} style={{ padding: '9px 18px', background: '#fff', color: '#374151', border: '1px solid #d1d5db', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>Clear</button>
                ...
                <div ref={historyRef} style={{ marginTop: 32, paddingTop: 20, borderTop: '1px dashed #e5e7eb' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                        <h3 style={{ margin: 0, fontSize: 15, fontWeight: 700, color: '#111827' }}>Command History</h3>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                            ...
                            <button onClick={loadHistory} title="Refresh"
                                style={{ padding: '6px 10px', background: '#f3f4f6', border: '1px solid #d1d5db', borderRadius: 8, cursor: 'pointer', fontSize: 13 }}>↻</button>
                        </div>
                    </div>
                        ...
                        <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 900 }}>
                            ...
                            <tbody>
                                {histLoading ? (
                                    ...
                                    <tr><td colSpan={HIST_COLS.length} style={{ ...TD, textAlign: 'center', padding: 48, color: '#94a3b8' }}>No commands sent yet.</td></tr>
                                ) : history.map(r => (
                                    <tr key={r.id}>
                                        <td style={TD}>{new Date(r.created_at).toLocaleString()}</td>
                                        <td style={{ ...TD, fontFamily: 'monospace', fontSize: 12 }}>{r.imei}</td>
                                        <td style={{ ...TD, fontFamily: 'monospace' }}>{r.content}</td>
                                        <td style={TD}><Badge text={r.message_format} color="#6366f1" /></td>
                                        <td style={TD}>{r.is_manual ? 'Yes' : 'No'}</td>
                                        <td style={TD}><Badge text={r.mode} color="#0ea5e9" /></td>
                                        <td style={TD}><Badge text={r.status} color={STATUS_COLOR[r.status] || '#9ca3af'} /></td>
                                        <td style={TD}>
                                            <button onClick={() => setViewingResponse(r)} title="View response"
                                                style={{ background: '#eef2ff', border: '1px solid #e0e7ff', borderRadius: 6, padding: '4px 8px', cursor: 'pointer', color: '#4f46e5' }}>👁</button>
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
            ...
            {viewingResponse && <ResponseModal record={viewingResponse} onClose={() => setViewingResponse(null)} />}
        </div>
    );
}
