    ...
    return (
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', background: '#fff' }}>
            <div style={{ padding: '14px 20px 12px', borderBottom: '1px solid #e5e7eb', flexShrink: 0 }}>
                <h2 style={{ margin: 0, fontSize: 16, fontWeight: 700, color: '#111827' }}>Face Recognition</h2>
                <p style={{ margin: '4px 0 0', fontSize: 12.5, color: '#6b7280' }}>Driver face-match checks captured live from device cameras (alert codes 1823/1824) — not a driving-behavior alert.</p>
            </div>

            <div style={{ display: 'flex', gap: 12, padding: '14px 20px 0', flexShrink: 0 }}>
                <StatCard label="Total Checks" value={total} />
                <StatCard label="Succeeded" value={succeeded} color="#16a34a" />
                <StatCard label="Failed" value={failed} color="#ef4444" />
            </div>

            <div style={{ display: 'flex', gap: 10, alignItems: 'flex-end', padding: '14px 20px', borderBottom: '1px solid #f1f5f9', flexShrink: 0 }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                    ...
                    <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Vehicle name, IMEI, or driver"
                        style={{ width: 260, boxSizing: 'border-box', padding: '7px 10px', border: '1px solid #d1d5db', borderRadius: 6, fontSize: 13, outline: 'none' }} />
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                    <label style={{ fontSize: 12, color: '#6b7280', fontWeight: 600 }}>Result</label>
                ...
                <button onClick={() => { setSearch(''); setResult(''); setStartDate(''); setEndDate(''); }}
                    style={{ padding: '7px 14px', background: '#fff', color: '#374151', border: '1px solid #d1d5db', borderRadius: 6, fontSize: 13, cursor: 'pointer' }}>Reset</button>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 860 }}>
                    ...
                    <tbody>
                        {loading ? (
                            ...
                            <tr key={ev.id}>
                                <td style={{ ...TD, color: '#6b7280', whiteSpace: 'nowrap' }}>{new Date(ev.occurred_at).toLocaleString()}</td>
