...
const rowFor = (dark) => ({ display: 'flex', justifyContent: 'space-between', gap: 16, padding: '8px 0', borderBottom: `1px solid ${dark ? '#1e293b' : '#f1f5f9'}`, fontSize: 13 });
const labelFor = (dark) => ({ color: dark ? '#94a3b8' : '#6b7280' });
const valueFor = (dark) => ({ color: dark ? '#f1f5f9' : '#111827', fontWeight: 600, textAlign: 'right' });
...
export default function DeviceDetailModal({ deviceId, onClose, dark }) {
    const [device,  setDevice]  = useState(null);
    const [loading, setLoading] = useState(true);
    const [error,   setError]   = useState('');
    const row = rowFor(dark);
    const label = labelFor(dark);
    const value = valueFor(dark);
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.35)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1100 }}>
            <div style={{ background: dark ? '#111827' : '#fff', borderRadius: 12, width: 420, maxHeight: '85vh', overflowY: 'auto', boxShadow: '0 24px 64px rgba(0,0,0,0.3)' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 20px', borderBottom: `1px solid ${dark ? '#1e293b' : '#f1f5f9'}` }}>
                    <h2 style={{ margin: 0, fontSize: 15, fontWeight: 700, color: dark ? '#f1f5f9' : '#0f172a' }}>Device Detail</h2>
                ...
                <div style={{ padding: '4px 20px 20px' }}>
                    {loading ? (
                        <p style={{ fontSize: 13, color: dark ? '#64748b' : '#94a3b8', textAlign: 'center', padding: '24px 0' }}>Loading…</p>
                    ) : error ? (
