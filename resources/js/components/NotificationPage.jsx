...
function ChannelsField({ allChannels, selected, onChange, disabled, dark }) {
    ...
    return (
        <div style={{ border: `1px solid ${dark ? '#334155' : '#d1d5db'}`, borderRadius: 8, padding: '7px 8px', display: 'flex', flexWrap: 'wrap', gap: 6, alignItems: 'center', minHeight: 38 }}>
            {selected.map(ch => (
                <span key={ch} style={tagStyle(dark)}>
                    {channelLabel(ch)}
...
function DevicesField({ allDevices, linkedIds, onAdd, onRemove, disabled, dark }) {
    ...
    return (
        <div style={{ border: `1px solid ${dark ? '#334155' : '#d1d5db'}`, borderRadius: 8, padding: '7px 8px', display: 'flex', flexWrap: 'wrap', gap: 6, alignItems: 'center', minHeight: 38 }}>
            {linked.map(d => (
                <span key={d.id} style={tagStyle(dark)}>
                    {d.name}
...
function AlarmsField({ selected, onChange, disabled, dark }) {
    const toggle = (key) => {
        if (disabled) return;
        onChange(selected.includes(key) ? selected.filter(k => k !== key) : [...selected, key]);
    };
    return (
        <div style={{ border: `1px solid ${dark ? '#334155' : '#d1d5db'}`, borderRadius: 8, maxHeight: 220, overflowY: 'auto' }}>
...
function NotificationModal({ notification, onClose, onSaved, dark }) {
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
            <div style={{ background: dark ? '#111827' : '#fff', borderRadius: 12, width: 440, maxHeight: '88vh', overflowY: 'auto', boxShadow: '0 24px 64px rgba(0,0,0,0.3)' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 20px', borderBottom: `1px solid ${dark ? '#1e293b' : '#f1f5f9'}` }}>
                    <h2 style={{ margin: 0, fontSize: 15, fontWeight: 700, color: dark ? '#f1f5f9' : '#0f172a' }}>{isNew ? 'New Notification' : 'Edit Notification'}</h2>
                        ...
                        <>
                            <div style={{ border: `1px solid ${dark ? '#1e293b' : '#e5e7eb'}`, borderRadius: 8, padding: 14, marginBottom: 14 }}>
                                ...
                                <div style={{ marginBottom: 14 }}>
                                    ...
                                    <select value={type} onChange={e => setType(e.target.value)} style={{ ...inputStyle(dark), cursor: 'pointer' }}>
                                ...
                                {type === 'alarm' && (
                                    <div style={{ marginBottom: 14 }}>
                                        <label style={fieldLabelStyle(dark)}>Alarms</label>
                                        <AlarmsField selected={alarms} disabled={saving} onChange={setAlarms} dark={dark} />
                                        {alarms.length > 0 && (
                                            <p style={{ margin: '6px 0 0', fontSize: 11.5, color: dark ? '#94a3b8' : '#6b7280' }}>{alarms.map(alarmLabel).join(', ')}</p>
                                        )}
                                    </div>
                                )}

                                <div style={{ marginBottom: 14 }}>
                                    <label style={fieldLabelStyle(dark)}>Channels</label>
                                    <ChannelsField allChannels={allChannels} selected={channels} disabled={saving}
                                        onChange={next => { setChannels(next); setTestResults(null); }} dark={dark} />
                                </div>

                                <button onClick={handleTest} disabled={testing} style={{ width: '100%', padding: 9, borderRadius: 7, border: `1.5px solid ${dark ? '#60a5fa' : '#3b82f6'}`, background: dark ? '#111827' : '#fff', color: dark ? '#60a5fa' : '#3b82f6', fontSize: 13, fontWeight: 600, cursor: testing ? 'not-allowed' : 'pointer', marginBottom: testResults ? 8 : 14 }}>
                                ...
                                {testResults && (
                                    <div style={{ marginBottom: 14, display: 'flex', flexDirection: 'column', gap: 4 }}>
                                        {testResults.map(r => (
                                            <div key={r.channel} style={{ fontSize: 12, color: r.success ? (dark ? '#4ade80' : '#15803d') : (dark ? '#fca5a5' : '#991b1b') }}>
                                ...
                                {!always && (
                                    <div style={{ marginTop: 14 }}>
                                        <label style={fieldLabelStyle(dark)}>Devices</label>
                                        {isNew ? (
                                            <p style={{ margin: 0, fontSize: 12, color: '#94a3b8' }}>Save the notification first, then reopen it to choose specific devices.</p>
                                        ) : (
                                            <DevicesField allDevices={allDevices} linkedIds={linkedDeviceIds} onAdd={handleAddDevice} onRemove={handleRemoveDevice} disabled={saving} dark={dark} />
                                        )}
                                    </div>
                                )}
                            </div>
                                ...
                                {extraOpen && (
                                    <div style={{ padding: 14, borderTop: `1px solid ${dark ? '#1e293b' : '#f1f5f9'}` }}>
                                        <div style={{ marginBottom: 14 }}>
                                            ...
                                            <input value={description} onChange={e => setDescription(e.target.value)} style={inputStyle(dark)} />
                                        </div>
                                        <div style={{ marginBottom: 14 }}>
                                            <label style={fieldLabelStyle(dark)}>Calendar</label>
                                            <select value={calendarId} onChange={e => setCalendarId(Number(e.target.value))} style={{ ...inputStyle(dark), cursor: 'pointer' }}>
                                                <option value={0}>None</option>
                                                {allCalendars.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                                            </select>
                                        </div>
                                        <div>
                                            <label style={fieldLabelStyle(dark)}>Saved Command</label>
                                            <select value={commandId} onChange={e => setCommandId(Number(e.target.value))} style={{ ...inputStyle(dark), cursor: 'pointer' }}>
                                                <option value={0}>None</option>
    ...
    return (
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', background: dark ? '#0b1220' : '#fff', position: 'relative' }}>
            <div style={{ padding: '14px 20px 12px', borderBottom: `1px solid ${dark ? '#1e293b' : '#e5e7eb'}`, flexShrink: 0 }}>
                <h2 style={{ margin: 0, fontSize: 16, fontWeight: 700, color: dark ? '#f1f5f9' : '#111827' }}>Notification</h2>
            </div>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 760 }}>
                    ...
                    <tbody>
                        {loading ? (
                            ...
                            <tr key={n.id} style={{ transition: 'background 0.1s' }} onMouseEnter={e => e.currentTarget.style.background = dark ? 'rgba(255,255,255,0.04)' : 'transparent'} onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
            ...
            {pendingDeleteId && (
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.35)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
                    <div style={{ background: dark ? '#111827' : '#fff', borderRadius: 12, padding: '24px 28px', width: 300, boxShadow: '0 16px 48px rgba(0,0,0,0.25)', textAlign: 'center' }}>
                        <h3 style={{ margin: '0 0 8px', fontSize: 15, fontWeight: 700, color: dark ? '#f1f5f9' : '#0f172a' }}>Delete notification?</h3>
