...
function GroupModal({ group, allGroups, onClose, onSaved, dark }) {
    const isNew = !group;
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
            <div style={{ background: dark ? '#111827' : '#fff', borderRadius: 12, width: 420, maxHeight: '88vh', overflowY: 'auto', boxShadow: '0 24px 64px rgba(0,0,0,0.3)' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 20px', borderBottom: `1px solid ${dark ? '#1e293b' : '#f1f5f9'}` }}>
                    <h2 style={{ margin: 0, fontSize: 15, fontWeight: 700, color: dark ? '#f1f5f9' : '#0f172a' }}>{isNew ? 'New Group' : 'Edit Group'}</h2>
                    ...
                    <div style={{ border: `1px solid ${dark ? '#1e293b' : '#e5e7eb'}`, borderRadius: 8, overflow: 'hidden', marginBottom: 14 }}>
                        ...
                        {extraOpen && (
                            <div style={{ padding: 14, borderTop: `1px solid ${dark ? '#1e293b' : '#f1f5f9'}` }}>
                                <label style={fieldLabelStyle(dark)}>Group</label>
                                <select value={parentId} onChange={e => setParentId(e.target.value)} style={{ ...inputStyle(dark), cursor: 'pointer' }}>
...
export default function GroupPage({ dark }) {
    ...
    return (
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', background: dark ? '#0b1220' : '#fff', position: 'relative' }}>
            <div style={{ padding: '14px 20px 12px', borderBottom: `1px solid ${dark ? '#1e293b' : '#e5e7eb'}`, flexShrink: 0 }}>
                <h2 style={{ margin: 0, fontSize: 16, fontWeight: 700, color: dark ? '#f1f5f9' : '#111827' }}>Groups</h2>
            </div>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    ...
                    <tbody>
                        {loading ? (
                            ...
                            <tr key={g.id} style={{ transition: 'background 0.1s' }} onMouseEnter={e => e.currentTarget.style.background = dark ? 'rgba(255,255,255,0.04)' : 'transparent'} onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
            ...
            {connGroup && <ConnectionsModal owner={connGroup} ownerType="group" onClose={() => setConnGroup(null)} dark={dark} />}
            ...
            {pendingDeleteId && (
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.35)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
                    <div style={{ background: dark ? '#111827' : '#fff', borderRadius: 12, padding: '24px 28px', width: 300, boxShadow: '0 16px 48px rgba(0,0,0,0.25)', textAlign: 'center' }}>
                        <h3 style={{ margin: '0 0 8px', fontSize: 15, fontWeight: 700, color: dark ? '#f1f5f9' : '#0f172a' }}>Delete group?</h3>
